...
TRADE_SIZE_USDT = 10
FEE_RATE = 0.001
STRATEGY_NAME = "V5_BTC_REGIME_BREAKOUT_PULLBACK_LONG_SHORT"
...
def compute_indicators(df):
    ...
    df["recent_long_breakout"] = df["long_breakout"].rolling(RECENT_BREAKOUT_CANDLES).max().astype(bool)
    df["recent_short_breakout"] = df["short_breakout"].rolling(RECENT_BREAKOUT_CANDLES).max().astype(bool)
    return df
...
def plot_trade(symbol, df, trade, trade_number, output_dir, candles_before, candles_after):
    ...
    price_ax.set_title(
        f"{symbol} V5 {trade['side']} trade {trade_number:04d} - {trade['result']} "
...
def parse_args():
    parser = argparse.ArgumentParser(description="Backtest V5 BTC-regime breakout-pullback strategy.")
    parser.add_argument(
        "--plot-trades",
        action="store_true",
    ...
    parser.add_argument(
        "--chart-dir",
        default="charts_v5_regime_breakout",
