...
TRADE_SIZE_USDT = 10
FEE_RATE = 0.001
STRATEGY_NAME = "V6_SHORT_BIASED_BREAKOUT_TRAIL"
...
LOSS_COOLDOWN_CANDLES = 10

LONG_RSI_RECOVERY = 50
LONG_RSI_MAX_ENTRY = 60
...
def backtest_pair(conn, cur, symbol, df, save_results=True):
    ...
    for index in range(1, len(df)):
        ...
        if position is None:
            ...
            if is_long_entry(row, previous_row):
        ...
        if position["side"] == "LONG":
            ...
            elif row["close"] < row["ema_trend"] and previous_row["close"] < previous_row["ema_trend"]:
        ...
        if trade["result"] == "LOSS":
            cooldown_until_index = index + LOSS_COOLDOWN_CANDLES
...
def plot_trade(symbol, df, trade, trade_number, output_dir, candles_before, candles_after):
    ...
    price_ax.set_title(
        f"{symbol} V6 {trade['side']} trade {trade_number:04d} - {trade['result']} "
...
def parse_args():
    parser = argparse.ArgumentParser(description="Backtest V6 short-biased breakout-pullback strategy.")
    parser.add_argument(
        "--plot-trades",
        action="store_true",
    ...
    parser.add_argument(
        "--chart-dir",
        default="charts_v6_short_biased",
