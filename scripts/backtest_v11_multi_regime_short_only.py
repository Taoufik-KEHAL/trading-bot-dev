...
TRADE_SIZE_USDT = 10
FEE_RATE = 0.001
STRATEGY_NAME = "V11_MULTI_REGIME_SHORT_ONLY"
...
def plot_trade(symbol, df, trade, trade_number, output_dir, candles_before, candles_after):
    ...
    price_ax.set_title(
        f"{symbol} V11 {trade['side']} {trade['regime']} trade {trade_number:04d} - {trade['result']} "
...
def parse_args():
    parser = argparse.ArgumentParser(description="Backtest V11 multi-regime short-only strategy.")
    parser.add_argument(
        "--timeframe",
        choices=["15min", "30min", "1h"],
        default=STRATEGY_TIMEFRAME,
        help="Strategy timeframe to resample the 5m candles into.",
    )
    parser.add_argument(
        "--plot-trades",
        action="store_true",
    ...
    parser.add_argument(
        "--chart-dir",
        default="charts_v11_multi_regime_short_only",
