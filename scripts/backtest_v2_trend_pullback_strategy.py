...
TRADE_SIZE_USDT = 10
STRATEGY_VERSION = "V2"
STRATEGY_NAME = f"{STRATEGY_VERSION}_EMA_TREND_PULLBACK_RSI_ATR"
...
def is_entry_signal(row, previous_row):
    ...
    pullback_recovered = (
        previous_row["close"] <= previous_row["ema_fast"]
        and row["close"] > row["ema_fast"]
    )
    rsi_recovered = (
        previous_row["rsi"] < RSI_RECOVERY_LEVEL
        and RSI_RECOVERY_LEVEL <= row["rsi"] <= RSI_MAX_ENTRY
    )
    enough_volume = row["volume"] >= row["volume_sma"] * 0.8

    return trend_is_bullish and pullback_recovered and rsi_recovered and enough_volume
...
def plot_trade(symbol, df, trade, trade_number, output_dir, candles_before, candles_after):
    ...
    price_ax.axhline(trade["take_profit_price"], color="#2ca02c", linestyle="--", linewidth=1, label="ATR target")
    price_ax.axhline(trade["stop_price"], color="#d62728", linestyle="--", linewidth=1, label="ATR/swing stop")
...
def parse_args():
    parser = argparse.ArgumentParser(description="Backtest an EMA trend-pullback strategy with ATR risk.")
    parser.add_argument(
        "--plot-trades",
        action="store_true",
