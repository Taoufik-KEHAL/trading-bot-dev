...
TRADE_SIZE_USDT = 100
FEE_RATE = 0.001
STRATEGY_NAME = "V12_100USDT_AGGRESSIVE_SHORT_EDGE"
...
HIGH_VOL_ATR_PERCENT = 0.012
LOW_VOL_ATR_PERCENT = 0.004

LONG_RSI_MIN_MOMENTUM = 55
LONG_RSI_MAX_MOMENTUM = 72
...
def load_all_market_data(cur, symbols):
    return {symbol: load_market_data(cur, symbol) for symbol in symbols}
...
def build_market_regime(market_data_by_symbol):
    ...
    for symbol, df in market_data_by_symbol.items():
        ...
        rows.append(
            df[["open_time", "close", "ema_mid", "ema_trend", "ema_mid_slope", "ema_trend_slope", "atr_percent"]]
    ...
    breadth["regime"] = "CHOP"
    low_volatility = breadth["avg_atr_percent"] < LOW_VOL_ATR_PERCENT
    high_volatility = breadth["avg_atr_percent"] >= HIGH_VOL_ATR_PERCENT
    bearish_market = (
        breadth["btc_bearish"]
    ...
    breadth.loc[low_volatility, "regime"] = "LOW_VOL_NO_TRADE"
    breadth.loc[bearish_market, "regime"] = "BEARISH_TREND"
    breadth.loc[bullish_market, "regime"] = "BULLISH_TREND"
    breadth.loc[bearish_market & high_volatility, "regime"] = "HIGH_VOL_BEARISH"

    return breadth


def add_market_regime(df, market_regime):
    return df.merge(market_regime, on="open_time", how="inner").dropna().reset_index(drop=True)
...
def is_long_entry(row, previous_row):
    ...
    fresh_breakout = row["long_breakout"]
    rsi_momentum = LONG_RSI_MIN_MOMENTUM <= row["rsi"] <= LONG_RSI_MAX_MOMENTUM
    volume = row["volume"] >= row["volume_sma"] * LONG_VOLUME_MULTIPLIER
    volatility = row["atr_percent"] >= MIN_ATR_PERCENT

    return (
        row["regime"] == "BULLISH_TREND"
        and fresh_breakout
        and trend
        and slope
        and rsi_momentum
...
def is_short_entry(row, previous_row):
    ...
    volume = row["volume"] >= row["volume_sma"] * SHORT_VOLUME_MULTIPLIER
    volatility = row["atr_percent"] >= MIN_ATR_PERCENT

    return (
        row["regime"] in ("BEARISH_TREND", "HIGH_VOL_BEARISH")
...
def build_position(symbol, df, index, row, side):
    if side == "LONG":
        ...
        hard_target_price = row["close"] + (risk_per_unit * LONG_HARD_TARGET_R)
    else:
        ...
        hard_target_price = row["close"] - (risk_per_unit * SHORT_HARD_TARGET_R)
...
def backtest_pair(conn, cur, symbol, df, save_results=True):
    ...
    for index in range(1, len(df)):
        ...
        if position is None:
            ...
            elif ENABLE_SHORTS and is_short_entry(row, previous_row):
...
def plot_trade(symbol, df, trade, trade_number, output_dir, candles_before, candles_after):
    ...
    price_ax.set_title(
        f"{symbol} V12 {trade['side']} {trade['regime']} trade {trade_number:04d} - {trade['result']} "
...
def parse_args():
    parser = argparse.ArgumentParser(description="Backtest V12 aggressive $100 short-edge strategy.")
    parser.add_argument(
        "--timeframe",
        choices=["15min", "30min", "1h"],
        default=STRATEGY_TIMEFRAME,
        help="Strategy timeframe to resample the 1m candles into.",
    )
    parser.add_argument(
        "--plot-trades",
        action="store_true",
    ...
    parser.add_argument(
        "--chart-dir",
        default="charts_v12_100usdt_aggressive_short",
...
def main():
    ...
    all_trades = []
    trades_by_symbol = {}
    market_data_by_symbol = load_all_market_data(cur, PAIRS)
    market_regime = build_market_regime(market_data_by_symbol)

    for symbol in symbols:
        symbol_df = add_market_regime(market_data_by_symbol[symbol], market_regime)
